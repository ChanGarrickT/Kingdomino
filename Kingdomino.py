...
if not e.get_valid_setup():
    ...
    run = True
    while run and not e.get_game_over():
        ...
        for p in e.get_players():
            ...
            if e.get_phase() == PLACE:
                discard_surf = pygame.image.load('images/discard_button.png')
                discard_rects.append(screen.blit(discard_surf, (BOARD_OFFSET[player_index][0] + 370,
        ...
        for i in range(len(e.get_deal())):
            ...
            deal_rects.append(screen.blit(deal_surf, (DEAL_OFFSET[0], DEAL_OFFSET[1] + i * 150)))
        ...
        # Indicate who's turn it is to claim
        if e.get_phase() == CLAIM and not e.get_game_over():
            turn_info = font.render(e.get_current_player().get_name() + '\'s turn to choose.', True, (0, 0, 0))
            screen.blit(turn_info, (DEAL_OFFSET[0], DEAL_OFFSET[1] + 600))
        # Add the Undo button
